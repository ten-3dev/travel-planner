...
export const EditmemberPage = () => {
    ...
    const update = async () => { // 회원 데이터를 수정 (이름, 연락처)
        if(window.confirm("수정하시겠습니까?")){
            if(isName & isPhone){
                try{
                    const data = await axios.post('http://192.168.52.16:8080/getUserUpdate',{name, tel: phone});
        ...
        if(window.confirm("수정하시겠습니까?")){
            if(isPassword & isPasswordConfirm){
                try{
                    ...
                    await axios.post('http://192.168.52.16:8080/getUserUpdatePw',{pw: createHashedPw, newPw: createHashedNewPw});
                    alert("비밀번호 변경 성공");
                }catch(e){
                    alert(e.response.data.msg);
                }
    ...
    const userDelete = async () => {
        if(window.confirm("정말로 탈퇴하시겠습니까??")){
            try{
                const data = await axios.delete('http://192.168.52.16:8080/userDelete');
                alert(data.data.msg);
                localStorage.clear();
    ...
    const onFileUpload = (fileList) => {
        ...
        axios.post('http://192.168.52.16:8080/uploadFile', formData);
        ...
        <MarginTopWrapper margin>
            ...
            <Styles.ProfileBox>
                <Styles.LeftProfileBox>
                    <Styles.ProfileImg src={sessionStorage.getItem("profileImg") ? `http://192.168.52.16:8080/image/view?value=${sessionStorage.getItem("profileImg")}` : "assets/defaultProfile.png"} />
                ...
                <Styles.MyProfileBox id="Profile">
                    ...
                    <Styles.BasicInformationBox>
                        <Styles.BasicInformationImg src={sessionStorage.getItem("profileImg") ? `http://192.168.52.16:8080/image/view?value=${sessionStorage.getItem("profileImg")}` : "assets/defaultProfile.png"} />
